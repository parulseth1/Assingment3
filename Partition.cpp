...
data* makeTree(vector<block> Blocks, int index, data* Parent, int LeftOrRightChild, int* order, int numOfBlocks, int* lb_best, int** left, int** right, int* count_node, int numNets) {
    ...
    if (index + 1 < numOfBlocks) {
        ...
        data* newnode_lb = new data;

        newnode_lb->parent = newNode;

        newnode_lb->blocknum = order[index];

        newnode_lb->RightOrLeftList = LEFT_CHILD; //set up the pointers,

        int Left_lb_count = calculateCO(newnode_lb, Blocks, numNets);

        newnode_lb->RightOrLeftList = RIGHT_CHILD; //set up the pointers,

        int Right_lb_count = calculateCO(newnode_lb, Blocks, numNets);



        if (Left_lb_count < Right_lb_count) {
            ...
            newNode->right = makeTree(Blocks, index + 1, newNode, RIGHT_CHILD, order, numOfBlocks, lb_best, left, right, count_node, numNets); //the left child is a new node, that is recursively created by the maketree function

            newNode->left = makeTree(Blocks, index + 1, newNode, LEFT_CHILD, order, numOfBlocks, lb_best, left, right, count_node, numNets);
        ...
        newNode->right = makeTree(Blocks, index + 1, newNode, RIGHT_CHILD, order, numOfBlocks, lb_best, left, right, count_node, numNets);
